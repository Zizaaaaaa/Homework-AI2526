...
def main() -> None:
    ...
    ap.add_argument("--n", type=int, required=True, help="Board size (e.g., 3 for 3x3, 4 for 4x4)")
    ap.add_argument("--state", type=str, required=True, help='State list with 0 as empty, e.g. "1,2,3,4,5,6,7,0,8"')
    ap.add_argument("--out", type=str, default="pddl/problem.pddl", help="Output path for problem.pddl")
    ap.add_argument("--name", type=str, default="npuzzle", help="Problem name prefix")
    args = ap.parse_args()

    state = parse_state(args.state)
    pddl = generate_problem_pddl(n=args.n, state=state, problem_name=args.name)
